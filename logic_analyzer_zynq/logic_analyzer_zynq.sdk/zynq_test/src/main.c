...
int main(){
	...
	char message[] = "Eluwina, byczq, inicjalizowansko samplera \n";

	XUartLite_Initialize(&uartHandle, XPAR_UARTLITE_0_DEVICE_ID);
	XUartLite_Send(&uartHandle, (void*)message, sizeof(message));
	...
	while(true){
		if(Sampler_Read(&value)){
				char buffer[50];
				while(XUartLite_IsSending(&uartHandle)); // wait for UART
				sprintf(buffer,"SAMPLE incoming from analyzer: %lx \n", value);
				XUartLite_Send(&uartHandle, (void*)buffer, strlen(buffer));
			}
	}
